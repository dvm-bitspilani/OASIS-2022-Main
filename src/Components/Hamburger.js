...
export default function Hamburger() {
    ...
    const staggerText = (node, node2, node3, node4) => {
        gsap.to([node, node2, node3, node4], { duration: 1, y: 0, opacity: 1, delay: 0.1, stagger: { amount: 0.3 }, ease: Power4.easeInOut })
    }
    const removeText = (node, node2, node3, node4) => {
        gsap.to([node, node2, node3, node4], { duration: 1, y: 100, opacity: 0, delay: 0.1, stagger: { amount: 0.3 }, ease: Power4.easeInOut })
    }
    const staggerDivs = (node, node2) => {
        gsap.to([node, node2], { duration: 1, y: 0, opacity: 1, delay: 0.1, stagger: { amount: 0.3 }, ease: Power4.easeInOut })
    }
    const removeDivs = (node, node2) => {
        gsap.to([node, node2], { duration: 1, y: 100, opacity: 0, delay: 0.1, stagger: { amount: 0.3 }, ease: Power4.easeInOut })
    }
    function toggleMenu() {
    ...
    <div ref={el => (container = el)} className={HamburgerCSS.container}>
        <div className={HamburgerCSS.hamContainer}>
            <div className={HamburgerCSS.hamBox} onClick={toggleMenu}>
                ...
                <div className={HamburgerCSS.ham3} id="ham3">
                </div>
            </div>
        </div>
